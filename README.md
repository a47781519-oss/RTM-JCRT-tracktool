...
两端**平行且横向错开**（或两端偏角反号）时会自动切到 **S 形解算器**，铺成

```
直线(可省) – 缓和 – 圆 – 缓和 – 直线(可省) – 缓和 – 圆 – 缓和 – 直线(可省)
```

两个圆曲线**转向相反、半径相同**，中间超高过 0。
S 形下**半径与缓和曲线长由解算器决定、不可调**（两个弯加三段直线只剩两个自由度，
再把半径固定就超定了），面板会把那两行收起来，**只留超高可调**。
...
| **连接模式** | 选中两个端点，自动解算「直线–缓和–圆–缓和–直线」把两端**精确闭合**（终点误差 0.000 m、朝向误差 0.000°）；判定为 S 形时自动切到反向曲线解算器 |
...
- S 形解算器下**半径与缓和曲线长不可调**（只能调超高）；两个弯的半径强制相等
- 超高上限 10°（原因见上）
