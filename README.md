...
- 面板「**撤销铺设**」或 `/tracktool undo`：撤掉最近一次铺好的轨道（默认保留 8 步，`maxUndo` 可调）。
  铺轨前清掉的别的轨道会一并还原；撤销后还会按世界实况复查一遍，把没进快照的残留底座清干净；
- 面板「**回退选点**」或 `/tracktool back`：只退掉最后一个选中的端点，**不会**碰已铺的轨道（没有选点时按钮是灰的）；
...
每段轨道铺下去之前，先扫描它**自己的路基范围**（RTM 要写的那些格子，每列往下 1 格、往上 2 格，
水平方向不外扩），把里面的旧 railbase 方块清掉再铺。必须先清：RTM 的 `setRail` 自己不清——
正好重合的格子被「接管」、旧轨道照样在；高一格或低一格的旧底座根本不在新线的方块表里，原样留在线上挡车。

| 扫到的东西 | 处理 | 能否撤销 |
|---|---|---|
| 无主底座（没 TE、指不到核心、核心是空的） | 清除 | 不还原（还原也只是一堵墙） |
| 别的活轨道落在新线范围里的底座 | 记录后清除 | 「撤销铺设」还原 |
| 核心压在新线上的旧轨道 | 整条记录后移除 | 「撤销铺设」还原 |
| 端点所在的既有轨道、本次已铺的段 | **不动** | — |

清了什么会在聊天栏报一行；新线切过了别的轨道时会报出它的核心坐标。开关：`preClearRoadbed`。
...
4. 点「确认」铺设；「撤销铺设」撤掉铺好的轨道（默认保留 8 步）
...
| `preClearRoadbed` | true | 铺轨前清掉新线路基范围里的旧 railbase 方块（见上文） |
