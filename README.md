...
每次铺完会自动扫一遍周围 50 格，把**找不到核心的**旧轨道底座方块清掉。
这种方块没有 TE 时碰撞箱是**整整一格的实心**，会把列车顶住（但不致脱轨）。
它们多半是拆旧轨道时核心区块没加载留下的——RTM 的 `breakBlock` 要先找到核心才能拆整条轨道。

**还连着核心的轨道永不会被动**，所以邻近的平行线、交叉线都是安全的。
...
| `roadbedSweepRadius` | 50 | 铺完自动清除周围多少格内的**无主**旧轨道路基（0 = 关闭） |
